describe Item do
  ...
  describe '#initialize' do
    ...
    it 'sets defaul values when its initailize with name and price' do
      expect(item.category).to eq(Item::Category::DEFAULT)
  ...
  describe '#tax' do
    ...
    it 'calculates import tax when the item is imported' do
      item = described_class.new(name: 'Item name', price: 10.0, category: Item::Category::FOOD, is_imported: true)
    ...
    it 'avoids sales tax and import tax when the category is different from DEFAULT and is_imported is false' do
      item = described_class.new(name: 'Item name', price: 10.0, category: Item::Category::FOOD)
